...
class AlbumModel : ViewModel() {
    ...
    // LiveData để theo dõi ảnh bị xóa
    private val _deletedPhotoUri = MutableLiveData<Uri?>()

    // Cập nhật LiveData khi ảnh bị đánh dấu là đã xóa
    fun markPhotoAsDeleted(uri: Uri) {
        Albums.markPhotoAsDeleted(uri)  // Cập nhật trong Albums object
        _deletedPhotoUri.postValue(uri) // Cập nhật LiveData để UI biết có thay đổi
    }
