...
fun Navigation(viewModel: AiGenerateImageViewModel, navController: NavHostController) {
    ...
    NavHost(navController = navController, startDestination = if (isLoggedIn) "gallery" else "login") {
        ...
        composable(
            "select_album_to_add_photo?photoUri={photoUri}",
            arguments = listOf(navArgument("photoUri") { type = NavType.StringType })
        ) { backStackEntry ->
            val albumModel: AlbumModel = hiltViewModel()
            val photoUri = backStackEntry.arguments?.getString("photoUri") ?: "" // Lấy URI ảnh
            SelectAlbumToAddPhoto(
                navController = navController,
                albumModel = albumModel,
        ...
        composable("trash_album_screen") {
            ...
            TrashAlbumScreen(navController = navController,viewModel = trashViewModel)
        }
